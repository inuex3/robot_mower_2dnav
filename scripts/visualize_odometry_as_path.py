...
def odom2_cb(data):
    global path2
    global num
    data.pose.pose.position.x = data.pose.pose.position.x * 0.76
    data.pose.pose.position.y = data.pose.pose.position.y * 0.76
    if num % 10 == 0:
...
odom_sub = rospy.Subscriber('/odometry/gps', Odometry, odom1_cb)
