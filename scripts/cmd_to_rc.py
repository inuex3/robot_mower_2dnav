...
class Publishsers():
    ...
    def make_msg(self, cmd_vel, odom):
        ...
        error_theta = cmd_vel.angular.z - theta_diff[2]/(self.current_time - self.prev_time)
        error_x = cmd_vel.linear.x - position_diff / (self.current_time - self.prev_time)
        D_error_theta = (error_theta - self.prev_error_theta) / (self.current_time - self.prev_time)
        D_error_x = (error_x - self.prev_error_x) / (self.current_time - self.prev_time)
        RC_theta = self.prev_RC_theta + error_theta * self.P_gain_theta - D_error_theta * self.D_gain_theta + self.I_error_theta * self.I_gain_theta
        RC_x = self.prev_RC_x + error_x * self.P_gain_x - D_error_x * self.D_gain_x + self.I_error_x * self.I_gain_x
        ...
        self.I_error_theta = self.I_error_theta + error_theta
        self.I_error_x = self.I_error_x + error_x

    def reset_I_error(self):
        self.I_error_x = 0
