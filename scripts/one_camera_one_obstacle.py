        ...
        for bbox in bboxes.bounding_boxes:
            if bbox.Class in self.obstacle_list:
                try:
                    ...
                    print(e)
            obstacle_msg.sort(key=lambda obstacle: obstacle.points.x)
            marker_data.sort(key=lambda markers: markers.pose.position.x)
            obstacle = obstacle_msg[0]
